...
\begin{document}
	...
	\begin{itemize}
		\item \textbf{CIFAR-100} consists of 60,000 32x32 images in 100 classes with 600 images per class. 
		\item \textbf{miniImagenet} consists of 60,000 images in 10 classes of approximately 256x256 pixels with 600 images per class.
	\end{itemize}
	...
	\begin{enumerate}
		...
		\item Generate weight updates using meta-leaner $\bm{\Delta}_M = M(\bm{\theta}_S, \bm{x}_{T,train})$
		\item Build target model by adding generated weight updates $\bm{\theta}_T = \bm{\theta}_S + \bm{\Delta}_M$
		\item (meta-training only) Perform backpropagation through meta-learner with examples from combined test sets $\bm{x}_{S,test} \cup \bm{x}_{T,test}$
	\end{enumerate}
	As during each episode the meta-learner sees not only a different model, but a different set of source and target classes and images, the meta-learner will learn to be invariant to each of these. I.e. it won't be dependent on a specific model, class collection, or image distribution. This allows for great knowledge to be obtained throughout training, and should provide great generalisation capability. \newline \newline
