...
\begin{document}
	...
	\TODO{Week 3}
	Transfer learning \TOCITE{} is a commonly employed technique of taking a neural network that has already been trained on a similar task, and adapting its weights to the new domain. For image classification, this is done by downloading a pre-trained model, replacing the final fully-connected Softmax output layer with a new output layer and training on the new data-set. The assumption that transfer learning hinges on is that the features learnt on a large data-set are general enough to \textit{transfer} tp a new, similar data-set. There are a number of well-known model architectures (section \TOCITE{}) available for download, typically being trained on the \textit{ImageNet} image database. \par
	If the model is trained on a sufficiently large data-set it is true that the learnt features are usually general enough to receive good results after introducing relatively few samples from the new domain. This is not always the case \TOCITE{a paper linked to something about imagenet's features not being general}, especially when the new domain is significantly different from the old. As this approach requires the training of a model on a large source data-set, the user is either stuck with an existing model and no option to change, or must first train their own custom model on the large data-set first. This is a long, costly process which in many cases can be prohibitive. \par 

	\subsection{Few-Shot Learning}
	Few-shot learning is the ability to learn a new concept or idea from only a ``few'' examples. In the context of image-classification it is the ability to learn a new class from only a few example images. \par
	\TODO{Week 3}
	\begin{itemize}
		\item Context of human learning
		\item Smaller datasets required
		\item rapid adaptation to new data
		\item what challenges are there?
		\item what's bad about it?
	...
	\TODO{Week 3}
	\begin{itemize}
		\item What is meta learning
		\item Relate to humans
		\item What is a meta learner
		\item Break into meta training, testing, episodes, etc.
	\end{itemize}
