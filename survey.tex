...
\begin{document}
	...
	\begin{itemize}
		\item \textbf{Optimization-based}: A model learns parameter update rules -- essentially replacing the role of an optimizer \TOCITE{} -- such that some good initial weights are modified rapidly and efficiently to perform for the current set of images.
		\item \textbf{Model-based}: A specific model architecture designed to handle few-shot problems by utilising some kind of memory unit -- typically an RNN (section \ref{rnn}) or memory-network \TOCITE{}.
		\item \textbf{Metric-based}: A model learns a representation of the given sample images and a method by which to compare query images with the recently-provided sample.
	\end{itemize}
	...
	\begin{figure}[!h]
		...
		\caption{A single few-shot training episode}
		\label{fig:episode:1}
	\end{figure}
