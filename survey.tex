...
\begin{document}
	...
	This technique used since the 1970s \cite{888} is aptly named \textit{backpropagation of error gradient}, as it involves the propagation of the gradient of the error -- or loss -- from the end of the network back. \par
	Returning to the concept of SGD (section \ref{sgd:1}) which was introduced on a conceptual level, we can now delve deeper into the application of the algorithm. Assuming that we have computed the gradient of each parameter in the network for the given examples in our batch, we can visualise this as a ``loss landscape'', whereby moving the parameters down-hill results in a reduction in the loss. We will consider a low-dimensional example, where we only have a \TODO{verify} single parameter (fig \TODO{make a figure}). \par
	Adjusting a parameter by the negative of the scaled gradient will result in a ``step'' the moves that parameter closer to a local minimum, with the objective to reach the lowest point possible. \\
	PROBLEMS:
	- Too large a step size
	- noise in the batches
