...
\begin{document}
	...
	\FEEDBACK{Can you say why it is so hard for for ANNs to not forget? Also at a high level what are the approaches people use to make ANNs not forget.}
	\begin{figure}[!h]
		\centering
	...
	As ANNs are modelled after the brain, I will briefly mention some of the known neurophysiological functions that allow the biological neural networks to adapt quickly to new information while retaining learnt information from previous tasks. \par
	\FEEDBACK{I don't like this sentence, it seems to suggest the existing neural networks already does continual learning since they are drawn by the above biology inspiration. I don't think that is the point you want to make. I think you need to say something like for continual learning we need to make ANNs more like the way the brain works but this is will be complex or something like that. }
	\begin{itemize}
	...
	\end{itemize}
	Memory networks have had significant success, and offer a practical alternative to RNNs when working with any variable-length data -- time-series based or not. \FEEDBACK{We is Memory Networks better than LSTMs?}
	\begin{figure}[h]
		\centering
